...
AUDIT_PRICE = {"currency": "EUR", "value": "0.50"}  # TIJDELIJK VERLAAGD OM TE TESTEN, terugzetten naar 79.00 na de test
...
def create_audit_payment(base_url, webshop_url, email):
...
def create_monitoring_signup(base_url, email, webshop_url):
    ...
    try:
        ...
        first_payment = customer.payments.create({
            ...
            "webhookUrl": f"{base_url}/webhooks/mollie",
            "sequenceType": "first",
            "metadata": {"type": "monitoring_first_payment", "webshop_url": webshop_url, "customer_id": customer.id, "email": email},
        })
